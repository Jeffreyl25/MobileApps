...
//  Breakout
//
//  Created by Jeffrey Lin on 7/7/23.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func makeBall() {
        ...
        addChild(ball) // adds ball obiect to the view
    }
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        for brick in bricks {
            if contact.bodyA.node == brick ||
                ...
                updateLabels()
                if brick.color == .red {
                    brick.color = .yellow // red bricks turn yellow
                }
                else if brick.color == .yellow { // yellow bricks turn green
                    brick.color = .green
                }
                else { //must be green so it gets removed
